...
class CommentsRepo {
  ...
  Future<Either<Failure, List<NewsCommentModel>>> getComments(
    ...
    try {
      ...
      final response = await apiService
          .get('/api/get_user_comments?news_id=$newsId', token: token);
  ...
  // Add a new comment
  Future<Either<Failure, NewsCommentModel>> addComment(
...
// Add a reply to a comment
  Future<Either<Failure, NewsCommentModel?>> addReplay(
  ...
  Future<Either<Failure, void>> deleteReplay(String commentId) async {
    try {
      final String? token = CacheHelper().getData(key: 'saveToken');
      if (token == null) return Left(ServerFailure('Token is null'));

      final response = await apiService.delete(
        '/api/delete_reply_comment',
        data: jsonEncode({'comment_id': commentId}),
        token: token,
      );
      print(response);
...
// Edit a comment
  Future<Either<Failure, NewsCommentModel>> editComment(
